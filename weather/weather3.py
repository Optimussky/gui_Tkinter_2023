...
def getWeather():
    ...
    name.config(text="Fecha/Hora Actual")
    ...
    if platform.system() == 'Windows':
        ...
        # Print data
        t.config(text=(ip))
        c.config(text=(str(comando),ip,"Ok"))
    else:
        ip = subprocess.getoutput("ifconfig | grep 'inet ' | grep -Fv 127.0.0.1 | awk '{print $1}'")
    #print(local)
